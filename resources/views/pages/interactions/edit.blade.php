        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('interactions.update', $interaction) }}" method="POST" class="space-y-4"
                                ...
                                <select name="staff_id" id="staff_id"
                                    ...
                                    @foreach ($staffs as $staff)
                                        <option value="{{ $staff->id }}"
                                            {{ old('staff_id', $interaction->user_id) == $staff->id ? 'selected' : '' }}>
                                            {{ $staff->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div>
                            ...
                            <select name="type" id="type"
                                ...
                                @foreach ($types as $type)
                                    <option value="{{ $type }}"
                                        {{ old('type', $interaction->type) == $type ? 'selected' : '' }}>
                                        {{ ucfirst($type) }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="datetime" id="datetime"
                                value="{{ old('datetime', \Carbon\Carbon::parse($interaction->datetime)->format('Y-m-d\TH:i')) }}"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200">
                            @error('datetime')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <textarea name="notes" id="notes" rows="3"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200">{{ old('notes', $interaction->notes) }}</textarea>
                            @error('notes')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="flex justify-end">
                            <button type="submit"
                                ...
                                x-text="loading ? 'Updating...' : 'Update'" x-bind:disabled="loading">
                            </button>
                        </div>
                    </form>
